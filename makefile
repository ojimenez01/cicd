...
clean:  ## 🧹 Clean up project
	rmdir /s /q $(SRC_DIR)\.venv
	rmdir /s /q tests\node_modules
	del /f /q tests\package*
	del /f /q test-results.xml
...
venv: $(SRC_DIR)\.venv\touchfile

$(SRC_DIR)\.venv\touchfile: $(SRC_DIR)\requirements.txt
	python -m venv $(SRC_DIR)\.venv
	call $(SRC_DIR)\.venv\Scripts\activate & pip install -Ur $(SRC_DIR)\requirements.txt
	type nul > $(SRC_DIR)\.venv\touchfile